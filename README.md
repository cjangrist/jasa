...
Firecrawl, GitHub, ScrapingBee, SerpAPI, and Serper are the currently integrated
usage sources; configured providers without an integration remain explicit as
`not_implemented`. Successful responses keep the provider's native JSON fields
under `raw`, with credentials and account identities recursively redacted.
Provider failures are isolated and reported beside the other records. GitHub
uses its unmetered authenticated rate-limit endpoint; ScrapingBee uses its free
usage endpoint; SerpAPI and Serper use their free account endpoints.
