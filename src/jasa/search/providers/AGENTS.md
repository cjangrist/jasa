...
| `codex.py` / `codex`           | `OPENAI_API_KEY`     | POST required, live Responses web-search tool | Gateway default `gpt-6-luna`; uses high context and unlimited returned-token budget; domains become both `filters` lists; rest re-rendered; cited URLs de-tracked. |
