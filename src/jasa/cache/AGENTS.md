...
- `base.py` — `CacheBackend`, `SearchCacheIdentity`, `make_cache_key`,
  `should_cache`, the v5 key prefix, and the default 129,600-second TTL.
...
The key is `jasa:search:v5:` plus SHA-256 of compact, sorted-key JSON. Identity
...
Values use a schema-v5 envelope containing the exact identity and complete
